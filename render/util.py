...
def compute_ssim(pred, target, mask=None, engine="skimage"):
    """Computes Masked SSIM following the neuralbody paper. must be np.ndarray"""
    ...
    if engine == "skimage":
        ...
        pred = pred[..., -1:].permute(2, 0, 1).unsqueeze(0)
        target = target[..., -1:].permute(2, 0, 1).unsqueeze(0)
        ssim = kornia.metrics.ssim(pred.cpu().numpy(), target.cpu().numpy(), window_size=7).mean()
    else:
        raise ValueError("Unknown SSIM engine: {}".format(engine))
    return ssim
...
def pixel_grid(width, height, center_x = 0.5, center_y = 0.5, device="cuda"):
    y, x = torch.meshgrid(
