...
def image_loss(img, target, loss='l1', tonemapper='none', use_python=False, reduction='mean'):
    ...
    '''
    assert reduction in ["mean", "none"], f"Invalid reduction type: {reduction}"

    if use_python:
        out = image_loss_fn(img, target, loss, tonemapper, reduction)
    else:
