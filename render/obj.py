...
def load_obj(filename, clear_ks=True, mtl_override=None):
    ...
    if len(used_materials) > 1:
        ...
        print("No materials found in obj file. Using default material.")
        uber_material = None
    else:
...
def write_obj(folder, mesh, save_material=True, save_uv=True, save_normal=True, save_v_color=False):
    obj_file = os.path.join(folder, 'mesh.obj')
    print(f"Writing mesh: {obj_file}")
    with open(obj_file, "w") as f:
        f.write("mtllib mesh.mtl\n")
        f.write("g default\n")

        v_pos = mesh.v_pos.detach().cpu().numpy() if mesh.v_pos is not None else None
        v_nrm = mesh.v_nrm.detach().cpu().numpy() if save_normal and mesh.v_nrm is not None else None
        v_tex = mesh.v_tex.detach().cpu().numpy() if save_uv and mesh.v_tex is not None else None
        v_color = mesh.v_color.detach().cpu().numpy() if save_v_color and mesh.v_color is not None else None

        if len(v_pos.shape) == 3:
            print(f"multiple meshes detected, only saving the first on in {v_pos.shape[0]} meshes")
            v_pos = v_pos[0] if v_pos is not None and  len(v_pos.shape) == 3 else v_pos
            v_nrm = v_nrm[0] if v_nrm is not None and  len(v_nrm.shape) == 3 else v_nrm
            v_tex = v_tex[0] if v_tex is not None and  len(v_tex.shape) == 3 else v_tex
            v_color = v_color[0] if v_color is not None and  len(v_color.shape) == 3 else v_color

        t_pos_idx = mesh.t_pos_idx.detach().cpu().numpy() if mesh.t_pos_idx is not None else None
        t_nrm_idx = mesh.t_nrm_idx.detach().cpu().numpy() if save_normal and mesh.t_nrm_idx is not None else None
        t_tex_idx = mesh.t_tex_idx.detach().cpu().numpy() if save_uv and mesh.t_tex_idx is not None else None
    ...
    if save_material:
        ...
        print(f"Writing material: {mtl_file}")
