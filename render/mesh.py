...
######################################################################################
class Mesh:
    def __init__(self, v_pos=None, t_pos_idx=None, v_nrm=None, t_nrm_idx=None, v_tex=None, t_tex_idx=None, v_tng=None, t_tng_idx=None, material=None, base=None, v_pos_canon=None, v_color=None):
        self.v_pos = v_pos
        self.v_nrm = v_nrm
        self.v_tex = v_tex
        self.v_tng = v_tng
        ...
        self.v_pos_canon = v_pos_canon
        self.v_color = v_color

        if base is not None:
            self.copy_none(base)
    
    DEVICE_KEYS = ["v_pos", "v_nrm", "v_tex", "v_tng", "t_pos_idx", "t_nrm_idx", "t_tex_idx", "t_tng_idx", "v_pos_canon"]
    def to(self, device):
    ...
    def copy_none(self, other):
        ...
        if self.v_pos_canon is None:
            self.v_pos_canon = other.v_pos_canon
        if self.v_color is None:
            self.v_color = other.v_color

    def clone(self, clone_v=True, clone_t=True, detach_v=True, detach_t=True):
        out = Mesh(base=self)
        if clone_v:
            ...
            if out.v_pos_canon is not None:
                out.v_pos_canon = out.v_pos_canon.clone()
            if out.v_color is not None:
                out.v_color = out.v_color.clone()
        if detach_v:
            if out.v_pos is not None:
                out.v_pos = out.v_pos.detach()
            if out.v_nrm is not None:
                out.v_nrm = out.v_nrm.detach()
            if out.v_tex is not None:
                out.v_tex = out.v_tex.detach()
            if out.v_tng is not None:
                out.v_tng = out.v_tng.detach()
            if out.v_pos_canon is not None:
                out.v_pos_canon = out.v_pos_canon.detach()
            if out.v_color is not None:
                out.v_color = out.v_color.detach()
        ...
        if detach_t:
            if out.t_pos_idx is not None:
                out.t_pos_idx = out.t_pos_idx.detach()
            if out.t_nrm_idx is not None:
                out.t_nrm_idx = out.t_nrm_idx.detach()
            if out.t_tex_idx is not None:
                out.t_tex_idx = out.t_tex_idx.detach()
            if out.t_tng_idx is not None:
                out.t_tng_idx = out.t_tng_idx.detach()
        return out

    
    EXTEND_KEYS = ["v_pos", "v_nrm", "v_tex", "v_tng", "v_pos_canon", "v_color"]
    def repeat(self, num_repeats: int = 1):
        out = self.clone(clone_v=True, clone_t=False, detach_v=False, detach_t=False)
        for key in self.EXTEND_KEYS:
            if out.__dict__[key] is not None:
                val = out.__dict__[key]
                if val.dim() == 2:
                    val = val.unsqueeze(0).repeat(num_repeats, 1, 1)
                elif val.dim() == 3:
                    val = val.repeat(num_repeats, 1, 1)
                else:
                    raise ValueError(f"v_pos must be 2D or 3D, got {val.dim()}")
...
def auto_normals(imesh):
    assert imesh.v_pos.dim() == 2, f"v_pos must be 2D, got {imesh.v_pos.shape}"
...
# Simple smooth vertex normal computation
######################################################################################
def batched_auto_normals(imesh):

    batch_size = imesh.v_pos.shape[0]
    assert imesh.v_pos.dim() == 3, f"Expected batched mesh, got {imesh.v_pos.shape}"
    ...
    # Splat face normals to vertices
    v_nrm = torch.zeros_like(imesh.v_pos)
    v_nrm.scatter_add_(-2, i0[None, :, None].repeat(batch_size,1,3), face_normals)
    v_nrm.scatter_add_(-2, i1[None, :, None].repeat(batch_size,1,3), face_normals)
    v_nrm.scatter_add_(-2, i2[None, :, None].repeat(batch_size,1,3), face_normals)
...
# Follows http://www.mikktspace.com/ conventions
######################################################################################
def batched_compute_tangents(imesh):
    assert imesh.v_pos.dim() == 3, f"Expected batched mesh, got {imesh.v_pos.shape}"
    ...
    batch_size = imesh.v_pos.shape[0]
    for i in range(0,3):
        idx = vn_idx[i][None, :, None].repeat(batch_size,1,3)
        tangents.scatter_add_(-2, idx, tang)                # tangents[n_i] = tangents[n_i] + tang
        tansum.scatter_add_(-2, idx, torch.ones_like(tang)) # tansum[n_i] = tansum[n_i] + 1
    tangents = tangents / tansum
