...
def batch_laplace_regularizer_const(v_pos, t_pos_idx):
    term = torch.zeros_like(v_pos)
    norm = torch.zeros_like(v_pos[..., 0:1])
    ...
    term.scatter_add_(-2, t_pos_idx[None, :, 0:1].repeat(batch_size, 1,3), (v1 - v0) + (v2 - v0))
    term.scatter_add_(-2, t_pos_idx[None, :, 1:2].repeat(batch_size, 1,3), (v0 - v1) + (v2 - v1))
    term.scatter_add_(-2, t_pos_idx[None, :, 2:3].repeat(batch_size, 1,3), (v0 - v2) + (v1 - v2))

    two = torch.ones_like(v0) * 2.0
    norm.scatter_add_(-2, t_pos_idx[:, 0:1].repeat(batch_size, 1, 1), two)
    norm.scatter_add_(-2, t_pos_idx[:, 1:2].repeat(batch_size, 1, 1), two)
    norm.scatter_add_(-2, t_pos_idx[:, 2:3].repeat(batch_size, 1, 1), two)
